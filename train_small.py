...
def train(args, opts):
    ...
    for name, param in model.named_parameters():
        if not (name.startswith('layers.18') or name.startswith('layers.19')
                or name.startswith('layers.20') or name.startswith('layers.21')
                or name.startswith('layers.22') or name.startswith('layers.23') 
                or name.startswith('layers.24') or name.startswith('layers.25')
