...
def train(args, opts):
    ...
    for name, param in model.named_parameters():
        if not (name.startswith('layers.8') or name.startswith('layers.9')
                or name.startswith('layers.10') or name.startswith('layers.11')
                or name.startswith('layers.12') or name.startswith('layers.13') 
    ...
    for epoch in range(epoch_start, args.epochs):
        ...
        best_epoch_checkpoint_path = os.path.join(opts.new_checkpoint, f'best_epoch_{epoch+1}.pth.tr')
        epoch_checkpoint_path = os.path.join(opts.new_checkpoint, f'epoch_{epoch+1}.pth.tr')
