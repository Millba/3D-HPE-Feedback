...
class FitHscDataset3D(Dataset):
    def __init__(self, keypoints_path, data_split, n_frames=243, stride=81, res_h=900, res_w=900, flip=False):
        ...
        self.data_list_2d, self.data_list_3d, self.data_list_camera = self.split_into_clips(data_2d, data_3d, self.n_frames, self.stride)
    ...
    def load_data(self, keypoints_path, data_split):
        ...
        for filename in os.listdir(split_path):
            if filename.endswith('_2D.npy'):
                ...
                # print(f"Loading files: {keypoints_2d_file}, {keypoints_3d_file}")  # 로그 추가
    ...
    def __getitem__(self, index):
        ...
        if self.data_split == 'train':
            if self.flip and random.random() > 0.5:
                keypoints_2d = flip_data(keypoints_2d)
                keypoints_3d = flip_data(keypoints_3d)

        assert not np.isnan(keypoints_2d).any(), "keypoints_2d have NaN."
        assert not np.isnan(keypoints_3d).any(), "keypoints_3d have NaN."
...
def _test(split):
    dataset = FitHscDataset3D(keypoints_path='../keypoints', data_split=split, flip=True)
