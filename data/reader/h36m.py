...
class DataReaderH36M(object):
    ...
    def read_2d(self):
        ...
        # print(f'h36m의 read_2d에서 정규화 전 trainset{trainset[0]}')
        # print(f'h36m의 read_2d에서 정규화 전 testset{testset[0]}')
        # map to [-1, 1]
    ...
    def read_3d(self):
        ...
        test_labels = self.dt_dataset['test']['joint3d_image'][::self.sample_stride, :, :3].astype(
            np.float32)  # [N, 17, 3]
        print(f'h36m의 read_3d에서 정규화 전 trainset{train_labels[0]}')
        print(f'h36m의 read_3d에서 정규화 전 testset{test_labels[0]}')
        # map to [-1, 1]
        for idx, camera_name in enumerate(self.dt_dataset['train']['camera_name']):
            if camera_name == '54138969' or camera_name == '60457274':
                res_w, res_h = 1000, 1002
                ...
                assert 0, '%d data item has an invalid camera name' % idx
            train_labels[idx, :, :2] = train_labels[idx, :, :2] / res_w * 2 - [1, res_h / res_w] # 2d norm
            train_labels[idx, :, 2:] = train_labels[idx, :, 2:] / res_w * 2 # 3d norm
